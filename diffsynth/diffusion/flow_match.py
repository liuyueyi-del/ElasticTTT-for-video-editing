...
class FlowMatchScheduler():
    ...
    def step(
        ...
        **kwargs,
        ...
        if sigma_map is not None:
            sigma_map = sigma_map.to(device=sample.device, dtype=sample.dtype)
            if sigma_next_map is None:
                sigma_next_map = torch.zeros_like(sigma_map)
            else:
                sigma_next_map = sigma_next_map.to(device=sample.device, dtype=sample.dtype)
            prev_sample = sample + model_output * (sigma_next_map - sigma_map)
        else:
    ...
    def add_noise(self, original_samples, noise, timestep, sigma_map=None):
        if isinstance(timestep, torch.Tensor):
            timestep = timestep.cpu()
        timestep_id = torch.argmin((self.timesteps - timestep).abs())
        sigma = self.sigmas[timestep_id]
        if sigma_map is not None:
            sigma_map = sigma_map.to(
                device=original_samples.device, dtype=original_samples.dtype
            )
            sample = (1 - sigma_map) * original_samples + sigma_map * noise
        else:
