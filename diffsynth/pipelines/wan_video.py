...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        reference_noise_mask_latents: Optional[torch.Tensor] = None,
        reference_fg_init_strength: Optional[float] = None,
        reference_bg_init_strength: Optional[float] = None,
        reference_strict_timestep_align: Optional[bool] = True,
        # Speech-to-video
        ...
        source_cfg_scale: Optional[float] = 0.0,
        ...
        # Scheduler
        denoising_strength_for_schedule = denoising_strength
        if reference_fg_init_strength is not None and reference_bg_init_strength is not None:
            ...
            denoising_strength_for_schedule = max(fg_strength, bg_strength)
        self.scheduler.set_timesteps(
            num_inference_steps,
            denoising_strength=denoising_strength_for_schedule,
            shift=sigma_shift,
        )
        ...
        inputs_shared = {
            ...
            "reference_noise_mask_latents": reference_noise_mask_latents,
            "reference_fg_init_strength": reference_fg_init_strength,
            "reference_bg_init_strength": reference_bg_init_strength,
            "reference_strict_timestep_align": bool(reference_strict_timestep_align),
        ...
        strict_align_enabled = bool(inputs_shared.get("reference_strict_timestep_align", False))
        latent_mask = inputs_shared.get("reference_latent_mask")
        fg_sigmas_schedule = None
        bg_sigmas_schedule = None
        if strict_align_enabled and latent_mask is None:
            warnings.warn(
                "reference_strict_timestep_align is enabled but no latent mask is available; falling back to global timestep mode."
            )
            strict_align_enabled = False
            inputs_shared["reference_strict_timestep_align"] = False
        if strict_align_enabled and latent_mask is not None:
            fg_raw = max(0.0, min(1.0, float(reference_fg_init_strength)))
            bg_raw = max(0.0, min(1.0, float(reference_bg_init_strength)))
            fg_sigmas_schedule, _ = self.scheduler.set_timesteps_fn(
                num_inference_steps=num_inference_steps,
                denoising_strength=fg_raw,
                shift=sigma_shift,
            )
            bg_sigmas_schedule, _ = self.scheduler.set_timesteps_fn(
                num_inference_steps=num_inference_steps,
                denoising_strength=bg_raw,
                shift=sigma_shift,
            )
            fg_sigmas_schedule = fg_sigmas_schedule.to(device=self.device, dtype=self.torch_dtype)
            bg_sigmas_schedule = bg_sigmas_schedule.to(device=self.device, dtype=self.torch_dtype)
            latent_mask = latent_mask.to(device=self.device, dtype=self.torch_dtype)

        source_context = None
        source_cfg_scale = float(source_cfg_scale or 0.0)
        if source_cfg_scale != 0.0 and source_prompt is not None and str(source_prompt).strip():
            self.load_models_to_device(["text_encoder"])
            ids, mask = self.tokenizer(source_prompt, return_mask=True, add_special_tokens=True)
            ids = ids.to(self.device)
            mask = mask.to(self.device)
            seq_lens = mask.gt(0).sum(dim=1).long()
            source_context = self.text_encoder(ids, mask)
            for i, v in enumerate(seq_lens):
                source_context[i, v:] = 0

        # Denoise
        self.load_models_to_device(self.in_iteration_models)
        models = {name: getattr(self, name) for name in self.in_iteration_models}
        num_train_timesteps = float(getattr(self.scheduler, "num_train_timesteps", 1000))
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if strict_align_enabled and fg_sigmas_schedule is not None and latent_mask is not None:
                fg_s = fg_sigmas_schedule[progress_id]
                bg_s = bg_sigmas_schedule[progress_id]
                sigma_map = latent_mask * fg_s + (1.0 - latent_mask) * bg_s
                if progress_id + 1 < len(fg_sigmas_schedule):
                    fg_s_next = fg_sigmas_schedule[progress_id + 1]
                    bg_s_next = bg_sigmas_schedule[progress_id + 1]
                    sigma_next_map = latent_mask * fg_s_next + (1.0 - latent_mask) * bg_s_next
                else:
                    sigma_next_map = torch.zeros_like(sigma_map)
                inputs_shared["sigma_map"] = sigma_map
                inputs_shared["sigma_next_map"] = sigma_next_map
                inputs_shared["timestep_map"] = sigma_map * num_train_timesteps
            else:
                inputs_shared.pop("sigma_map", None)
                inputs_shared.pop("sigma_next_map", None)
                inputs_shared.pop("timestep_map", None)
            
            # Inference
            noise_pred_posi = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep)
            noise_pred_target = noise_pred_posi
            if cfg_scale != 1.0:
                if cfg_merge:
            ...
            if source_context is not None and source_cfg_scale != 0.0:
                if cfg_merge:
                    source_inputs_shared = dict(inputs_shared)
                    source_inputs_shared["cfg_merge"] = False
                    for name in ("context", "clip_feature", "y", "reference_latents"):
                        tensor = source_inputs_shared.get(name)
                        if tensor is not None and hasattr(tensor, "shape") and tensor.shape[0] % 2 == 0:
                            source_inputs_shared[name] = tensor[: tensor.shape[0] // 2]
                    source_inputs_shared["context"] = source_context
                    noise_pred_src = self.model_fn(**models, **source_inputs_shared, timestep=timestep)
                else:
                    source_inputs_posi = dict(inputs_posi)
                    source_inputs_posi["context"] = source_context
                    noise_pred_src = self.model_fn(
                        **models, **inputs_shared, **source_inputs_posi, timestep=timestep
                    )
                noise_pred = noise_pred + source_cfg_scale * (noise_pred_target - noise_pred_src)

            # Scheduler
            if strict_align_enabled and inputs_shared.get("sigma_map") is not None:
                inputs_shared["latents"] = self.scheduler.step(
                    noise_pred,
                    self.scheduler.timesteps[progress_id],
                    inputs_shared["latents"],
                    sigma_map=inputs_shared.get("sigma_map"),
                    sigma_next_map=inputs_shared.get("sigma_next_map"),
                )
            else:
                inputs_shared["latents"] = self.scheduler.step(
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    @staticmethod
    def _apply_wan_shift(sigma_raw, shift):
        """Apply Wan nonlinear sigma shift: sigma_shifted = shift * s / (1 + (shift-1)*s)."""
        if shift is None or shift <= 0:
            return sigma_raw
        return shift * sigma_raw / (1.0 + (shift - 1.0) * sigma_raw)
    ...
    def process(
        ...
        if pipe.scheduler.training:
            ...
            if (
                bool(reference_strict_timestep_align)
                and hasattr(pipe.scheduler, "sigmas")
                and reference_noise_mask_latents is not None
                and reference_fg_init_strength is not None
                and reference_bg_init_strength is not None
                and hasattr(pipe.scheduler, "timesteps")
            ):
                mask = reference_noise_mask_latents
                if mask.ndim == 4:
                    mask = mask.unsqueeze(1)
                if mask.ndim == 5 and mask.shape[1] != 1:
                    mask = mask[:, :1]
                mask = mask.to(device=input_latents.device, dtype=input_latents.dtype)
                target_shape = (
                    int(input_latents.shape[2]),
                    int(input_latents.shape[3]),
                    int(input_latents.shape[4]),
                )
                if (
                    int(mask.shape[2]) != target_shape[0]
                    or int(mask.shape[3]) != target_shape[1]
                    or int(mask.shape[4]) != target_shape[2]
                ):
                    mask = torch.nn.functional.interpolate(
                        mask, size=target_shape, mode="nearest"
                    )
                mask = mask.clamp(0.0, 1.0)

                # Apply nonlinear shift to get correct sigma values
                fg_raw = max(0.0, min(1.0, float(reference_fg_init_strength)))
                bg_raw = max(0.0, min(1.0, float(reference_bg_init_strength)))
                shift_val = float(sigma_shift) if sigma_shift is not None else 5.0
                fg_sigma = self._apply_wan_shift(fg_raw, shift_val)
                bg_sigma = self._apply_wan_shift(bg_raw, shift_val)

                sigma_map = (
                    mask * fg_sigma + (1.0 - mask) * bg_sigma
                ).to(dtype=input_latents.dtype, device=input_latents.device)
                latents = pipe.scheduler.add_noise(
                    input_latents,
                    noise,
                ...
                output["reference_latent_mask"] = mask
            return output
...
def model_fn_wan_video(
    ...
    timestep_map: Optional[torch.Tensor] = None,
    reference_strict_timestep_align: bool = False,
    **kwargs,
):
    if sliding_window_size is not None and sliding_window_stride is not None:
        model_kwargs = dict(
            ...
            reference_strict_timestep_align=reference_strict_timestep_align,
        )
        tensor_names = ["latents", "y"]
        if timestep_map is not None:
            tensor_names.append("timestep_map")
    ...
    strict_align_enabled = bool(reference_strict_timestep_align and timestep_map is not None)
    t = None
    t_mod = None
    if not strict_align_enabled:
        # Timestep
        if dit.seperated_timestep and fuse_vae_embedding_in_latents:
            # Extract a scalar timestep: in training the batch repeats the same value.
            _ts = timestep.reshape(-1)[0] if isinstance(timestep, torch.Tensor) and timestep.numel() > 1 else timestep
            timestep = torch.concat([
                torch.zeros((1, latents.shape[3] * latents.shape[4] // 4), dtype=latents.dtype, device=latents.device),
                torch.ones((latents.shape[2] - 1, latents.shape[3] * latents.shape[4] // 4), dtype=latents.dtype, device=latents.device) * _ts
            ]).flatten()
            ...
            # Expand t_mod to match the actual batch size (e.g. batch=2 in training).
            if t_mod.shape[0] < context.shape[0]:
                t_mod = t_mod.expand(context.shape[0], -1, -1, -1)
        else:
            t = dit.time_embedding(sinusoidal_embedding_1d(dit.freq_dim, timestep))
            t_mod = dit.time_projection(t).unflatten(1, (6, dit.dim))
    
    # Motion Controller
    if (not strict_align_enabled) and motion_bucket_id is not None and motion_controller is not None:
    ...
    if strict_align_enabled and timestep_map is not None and timestep_map.shape[0] != context.shape[0]:
        timestep_map = torch.concat([timestep_map] * context.shape[0], dim=0)
    ...
    if reference_latents is not None:
        ...
        f += 1
    if strict_align_enabled and timestep_map is not None:
        tmap = timestep_map
        if tmap.ndim == 4:
            tmap = tmap.unsqueeze(1)
        tmap = tmap.to(dtype=latents.dtype, device=latents.device)
        token_timestep = torch.nn.functional.interpolate(
            tmap, size=(f_without_ref, h, w), mode="nearest"
        ).reshape(tmap.shape[0], -1)
        if reference_latents is not None:
            ref_tokens = int(reference_latents.shape[1])
            ref_timestep = timestep.view(-1, 1).to(dtype=token_timestep.dtype)
            ref_timestep = ref_timestep.repeat(1, ref_tokens)
            token_timestep = torch.concat([ref_timestep, token_timestep], dim=1)
        t = dit.time_embedding(
            sinusoidal_embedding_1d(
                dit.freq_dim, token_timestep.reshape(-1)
            ).view(token_timestep.shape[0], token_timestep.shape[1], -1)
        )
        t_mod = dit.time_projection(t).unflatten(2, (6, dit.dim))
        if motion_bucket_id is not None and motion_controller is not None:
            motion_t = motion_controller(motion_bucket_id).unflatten(1, (6, dit.dim)).unsqueeze(1)
            t_mod = t_mod + motion_t
